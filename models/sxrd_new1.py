...
class Sample:
    ...
    def calc_f_layered_sorbate_muscovite(self,h,k,l,args,height_offset=0):
        ...
        if h[0]==0 and k[0]==0:#layered structure has effect only on specular rod
            ...
            Auc=self.unit_cell.a*self.unit_cell.b*np.sin(self.unit_cell.gamma)
            q=2*np.pi*dinv
            F_layered_sorbate=f*(Auc*1*density_s)*np.exp(-0.5*q**2*u0_s**2)*np.exp(q*(first_layer_height_s+53.6523273+height_offset)*1.0J)\
        ...
        for domain_index in range(len(slabs['domains'])):
            ...
            for i in range(resolution):
                ...
                if z_each>(53.6523273+height_offset):
                    bulk_water=1
                eden[-1]=eden[-1]+np.sum(10*wt*water_density*layered_water[2]*(2*np.pi*np.array(sigma_layered_water)**2)**-0.5*np.exp(-0.5/np.array(sigma_layered_water)**2*(z_each-np.array(z_layered_water))**2))+(.33-0.16233394)*wt*bulk_water
                eden_layer_water.append(np.sum(10*wt*water_density*layered_water[2]*(2*np.pi*np.array(sigma_layered_water)**2)**-0.5*np.exp(-0.5/np.array(sigma_layered_water)**2*(z_each-np.array(z_layered_water))**2))+(.33-0.16233394)*wt*bulk_water)
